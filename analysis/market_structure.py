...
def get_full_market_ctx(df_1m: pl.DataFrame, df_15m: pl.DataFrame, df_1h: pl.DataFrame) -> dict:
    ...
    trend_15m = regime_15m["trend"]
    priors = {"LONG": 30.0, "SHORT": 30.0}
    if trend_15m == "BULLISH":   priors["LONG"] = 40.0; priors["SHORT"] = 20.0
    elif trend_15m == "BEARISH": priors["LONG"] = 20.0; priors["SHORT"] = 40.0

    return {
        "trend_15m":  trend_15m, "zone_15m":   regime_15m["zone"],
        "trend_1h":   regime_1h["trend"], "zone_1h":    regime_1h["zone"],
        "priors":     priors, "sweep":      sweep_1m,
        "bos_choch":  bos_choch, "fvg_1m":     fvg_1m,
        "levels":     levels_15m, "ob_touch":   ob_touch, "eqh_eql":    eqh_eql,
    }